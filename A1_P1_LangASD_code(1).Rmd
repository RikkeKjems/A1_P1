...
#less than 1.5
child_MLU2=filter(finalData,CHI_MLU<1.5,Visit==1)
length(CHI_MLU)
child_MLU$ID
unique(CHI_MLU$ID)

#Not all trials
#Function for visit pr ID then we can filter afterwards

missed = finalData %>% group_by(ID) %>% tally()
filter(missed, n < 6)
...
finalData = mutate(finalData, GrandChildAvg = mean(tokens_CHI))

finalData$ID=as.factor(finalData$ID)


tokensAllVisits = finalData %>% group_by(ID) %>%dplyr::summarise(meanWordsTotal = mean(tokens_CHI))

mergedData = merge(finalData, tokensAllVisits)
...
tokensaverage= mergedData %>% group_by(ID) %>%summarise(meanWordsChild = mean(tokens_CHI))

mergedData2 = merge(mergedData, tokensaverage, by = "ID" )
